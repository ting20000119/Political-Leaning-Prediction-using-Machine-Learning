...
def getdeminfo():
    cur = conn.cursor()
    insert_stmt = (
        "SELECT author,subreddit,score from testdemminusrep where (epoch > 1577883600) LIMIT 5000"
        )
    cur.execute(insert_stmt)
    result = cur.fetchall()
    cur.close()
...
def getrepinfo():
    cur = conn.cursor()
    insert_stmt = (
        "SELECT author,subreddit,score from testrepminusdem where (epoch > 1577883600) LIMIT 5000"
        )
    cur.execute(insert_stmt)
    result = cur.fetchall()
    cur.close()
